...
export default function DailyChallenge({ attemptsToday, claimed, dailyTotal = 10 }: { attemptsToday: number, claimed: boolean, dailyTotal?: number }) {
  const router = useRouter();
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState('');

  const goal = Math.max(1, dailyTotal);
  ...
  return (
    <div className="bg-white rounded-3xl shadow-sm border border-gray-100 p-6 mb-6">
      ...
      <p className="text-gray-500 text-sm mb-4">Termine tes <span className="font-bold text-gray-700">{goal} cas du jour</span> pour débloquer la récompense.</p>
